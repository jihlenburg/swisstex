...
def test_doctrine_lengths_in_log(tmp_path):
    ...
    # returncode assert comes FIRST: without it a build that died before the
    # typeouts would fail on an inscrutable "no match" instead of naming the
    # real breakage (the log is still written on a failed run).
    r = build_doc(ROOT / "tests/fixtures/plain.tex", tmp_path)
    assert r.returncode == 0, r.log[-2000:]
